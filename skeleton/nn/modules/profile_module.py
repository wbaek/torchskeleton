...
class ProfileModule(torch.nn.Module):
    def __init__(self):
        super(ProfileModule, self).__init__()
        self.handles = []
        self.operation_flops = []

    def count_parameters(self, name_filter=lambda name: True):
        return np.sum(params.numel() for name, params in self.named_parameters() if name_filter(name))

    def count_flops(self, name_filter=lambda name: True):
        return np.sum([data['flops'] for data in self.operation_flops if name_filter(data['name'])])

    def register_profile_hooks(self, module_filter=lambda name: 'skeleton' not in name and 'loss' not in name):
        def get_hook(name):
            def counting(module, inputs, outputs):
                class_name = module.__class__.__name__

                flops = 0
                module_type = type(module)
                if not module_filter(str(module_type)):
                    pass
                elif module_type in COUNT_FN_MAP:
                    fn = COUNT_FN_MAP[module_type]
                    flops = fn(module, inputs, outputs) if fn is not None else 0
                else:
                ...
                self.operation_flops.append(data)
            return counting
