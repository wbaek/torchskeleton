...
def test_cifar_dataset():
    _, _, data_shape = Cifar.sets(1, num_classes=10)
    assert data_shape == [(1, 3, 32, 32), (1,)]

    _, _, data_shape = Cifar.sets(1, num_classes=100)
    assert data_shape == [(1, 3, 32, 32), (1,)]
...
def test_cifar_dataset_cv_split():
    train_loader, valid_loader, test_loader, _ = Cifar.loader(1, num_classes=10, cv_ratio=0.1)
    assert len(train_loader) == 45000
    assert len(valid_loader) == 5000
    assert len(test_loader) == 10000

    train_loader, valid_loader, test_loader, _ = Cifar.loader(1, num_classes=10, cv_ratio=0.2)
    assert len(train_loader) == 40000
    assert len(valid_loader) == 10000
    assert len(test_loader) == 10000

    train_loader, valid_loader, test_loader, _ = Cifar.loader(1, num_classes=10, cv_ratio=0.5)
    assert len(train_loader) == 25000
    assert len(valid_loader) == 25000
    assert len(test_loader) == 10000

    train_loader, valid_loader, test_loader, _ = Cifar.loader(1, num_classes=10, cv_ratio=0.9)
    assert len(train_loader) == 5000
    assert len(valid_loader) == 45000
    assert len(test_loader) == 10000

    train_loader, valid_loader, test_loader, _ = Cifar.loader(32, num_classes=10, cv_ratio=0.1)
